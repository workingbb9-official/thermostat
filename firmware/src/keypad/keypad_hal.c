...
const char key_map[KEYPAD_HAL_ROWS][KEYPAD_HAL_COLS] = {
    {'1', '2', '3', 'A'},
    {'4', '5', '6', 'B'},
    {'7', '8', '9', 'C'},
    {'*', '0', '#', 'D'},
...
{
    for (int8_t row = 0; row < KEYPAD_HAL_ROWS - 1; ++row) {
...
{
    static const uint8_t col_bits[KEYPAD_HAL_COLS] = {
        PORTB1,
        PORTB2,
        PORTB3,
        PORTB4,
    ...
    // Omit PINB0 because it sits on a different register
    static const uint8_t row_bits[KEYPAD_HAL_ROWS - 1] = {
        PIND5,
        PIND6,
        PIND7,
    };

    for (uint8_t col = 0; col < KEYPAD_HAL_COLS; ++col) {
        CLR_BIT(PORTB, col_bits[col]);

        // Check all PIND
        int8_t pressed_row = check_rows(row_bits);
        if (pressed_row != NO_ROW) {
