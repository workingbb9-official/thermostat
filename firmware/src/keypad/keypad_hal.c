...
char keypad_hal_read(void) {
    static const uint8_t col_bits[KEYPAD_COLS] = 
        {PORTB1, PORTB2, PORTB3, PORTB4};

    // Omit PINB0 because it sits on a different register
    static const uint8_t row_bits[KEYPAD_ROWS - 1] = {PIND5, PIND6, PIND7};
...
static int8_t check_rows(const uint8_t *row_bits) {
    for (int8_t row = 0; row < KEYPAD_ROWS - 1; ++row) {
